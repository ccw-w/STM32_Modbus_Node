...
/**
 * @brief  Deinitializes the DAC peripheral registers to their default reset
 * values.
 * @param  None
 * @retval None
 */
...
void DAC_Init(uint32_t DAC_Channel, DAC_InitTypeDef *DAC_InitStruct) {
  ...
  /* Configure for the selected DAC channel: buffer output, trigger, wave
     generation, mask/amplitude for wave generation */
 ...
 *     @arg DAC_Channel_1: DAC Channel1 selected
 *     @arg DAC_Channel_2: DAC Channel2 selected
 * @param  DAC_IT: specifies the DAC interrupt sources to be enabled or
 * disabled. This parameter can be the following values:
...
ITStatus DAC_GetITStatus(uint32_t DAC_Channel, uint32_t DAC_IT) {
  ...
  if (((DAC->SR & (DAC_IT << DAC_Channel)) != (uint32_t)RESET) &&
      enablestatus) {
    /* DAC_IT is set */
    bitstatus = SET;
  } else {
