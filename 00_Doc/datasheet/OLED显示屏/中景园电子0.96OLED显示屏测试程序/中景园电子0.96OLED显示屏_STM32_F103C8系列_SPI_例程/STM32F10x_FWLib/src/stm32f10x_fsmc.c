...
/**
 * @brief  Deinitializes the FSMC NAND Banks registers to their default reset
 * values.
 * @param  FSMC_Bank: specifies the FSMC Bank to be used
 *   This parameter can be one of the following values:
...
/**
 * @brief  Deinitializes the FSMC PCCARD Bank registers to their default reset
 * values.
 * @param  None
 * @retval None
 */
...
void FSMC_NORSRAMInit(FSMC_NORSRAMInitTypeDef *FSMC_NORSRAMInitStruct) {
  ...
  assert_param(IS_FSMC_WAIT_SIGNAL_ACTIVE(
  ...
  assert_param(
      IS_FSMC_TURNAROUND_TIME(FSMC_NORSRAMInitStruct->FSMC_ReadWriteTimingStruct
  ...
  /* Bank1 NOR/SRAM timing register for write configuration, if extended mode is
   * used */
...
void FSMC_NANDInit(FSMC_NANDInitTypeDef *FSMC_NANDInitStruct) {
  ...
  assert_param(
      IS_FSMC_WAIT_TIME(FSMC_NANDInitStruct->FSMC_AttributeSpaceTimingStruct
                            ->FSMC_WaitSetupTime));
  assert_param(
      IS_FSMC_HOLD_TIME(FSMC_NANDInitStruct->FSMC_AttributeSpaceTimingStruct
                            ->FSMC_HoldSetupTime));
  assert_param(IS_FSMC_HIZ_TIME(
  ...
  tmppcr = (uint32_t)FSMC_NANDInitStruct->FSMC_Waitfeature |
           PCR_MemoryType_NAND | FSMC_NANDInitStruct->FSMC_MemoryDataWidth |
  ...
  /* Set tmppatt value according to FSMC_AttributeSpaceTimingStructure
   * parameters */
  tmppatt =
      (uint32_t)
          FSMC_NANDInitStruct->FSMC_AttributeSpaceTimingStruct->FSMC_SetupTime |
...
void FSMC_PCCARDInit(FSMC_PCCARDInitTypeDef *FSMC_PCCARDInitStruct) {
  ...
  assert_param(
      IS_FSMC_WAIT_TIME(FSMC_PCCARDInitStruct->FSMC_AttributeSpaceTimingStruct
                            ->FSMC_WaitSetupTime));
  assert_param(
      IS_FSMC_HOLD_TIME(FSMC_PCCARDInitStruct->FSMC_AttributeSpaceTimingStruct
                            ->FSMC_HoldSetupTime));
  assert_param(
      IS_FSMC_HIZ_TIME(FSMC_PCCARDInitStruct->FSMC_AttributeSpaceTimingStruct
                           ->FSMC_HiZSetupTime));
  assert_param(IS_FSMC_SETUP_TIME(
  ...
  /* Set PMEM4 register value according to FSMC_CommonSpaceTimingStructure
   * parameters */
  FSMC_Bank4->PMEM4 =
      (uint32_t)
          FSMC_PCCARDInitStruct->FSMC_CommonSpaceTimingStruct->FSMC_SetupTime |
  ...
  /* Set PATT4 register value according to FSMC_AttributeSpaceTimingStructure
   * parameters */
  FSMC_Bank4->PATT4 =
 ...
 *     @arg FSMC_Bank1_NORSRAM3: FSMC Bank1 NOR/SRAM3
 *     @arg FSMC_Bank1_NORSRAM4: FSMC Bank1 NOR/SRAM4
 * @param  NewState: new state of the FSMC_Bank. This parameter can be: ENABLE
 * or DISABLE.
 * @retval None
 */
void FSMC_NORSRAMCmd(uint32_t FSMC_Bank, FunctionalState NewState) {
  ...
  if (NewState != DISABLE) {
    /* Enable the selected NOR/SRAM Bank by setting the PBKEN bit in the BCRx
     * register */
    FSMC_Bank1->BTCR[FSMC_Bank] |= BCR_MBKEN_Set;
  } else {
    /* Disable the selected NOR/SRAM Bank by clearing the PBKEN bit in the BCRx
     * register */
...
void FSMC_NANDCmd(uint32_t FSMC_Bank, FunctionalState NewState) {
  ...
  if (NewState != DISABLE) {
    /* Enable the selected NAND Bank by setting the PBKEN bit in the PCRx
     * register */
    if (FSMC_Bank == FSMC_Bank2_NAND) {
    ...
    /* Disable the selected NAND Bank by clearing the PBKEN bit in the PCRx
     * register */
    if (FSMC_Bank == FSMC_Bank2_NAND) {
...
void FSMC_NANDECCCmd(uint32_t FSMC_Bank, FunctionalState NewState) {
  ...
  if (NewState != DISABLE) {
    /* Enable the selected NAND Bank ECC function by setting the ECCEN bit in
     * the PCRx register */
    if (FSMC_Bank == FSMC_Bank2_NAND) {
    ...
    /* Disable the selected NAND Bank ECC function by clearing the ECCEN bit in
     * the PCRx register */
    if (FSMC_Bank == FSMC_Bank2_NAND) {
 ...
 * @param  FSMC_IT: specifies the FSMC interrupt sources to be enabled or
