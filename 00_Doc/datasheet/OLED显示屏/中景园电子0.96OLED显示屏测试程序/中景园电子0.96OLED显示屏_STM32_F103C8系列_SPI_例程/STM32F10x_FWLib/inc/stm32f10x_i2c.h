...
typedef struct {
  ...
  uint16_t I2C_AcknowledgedAddress; /*!< Specifies if 7-bit or 10-bit address is
                                       acknowledged. This parameter can be a
                                       value of @ref I2C_acknowledged_address */
} I2C_InitTypeDef;
...
#define IS_I2C_GET_IT(IT)                                                      \
  (((IT) == I2C_IT_SMBALERT) || ((IT) == I2C_IT_TIMEOUT) ||                    \
   ((IT) == I2C_IT_PECERR) || ((IT) == I2C_IT_OVR) || ((IT) == I2C_IT_AF) ||   \
   ((IT) == I2C_IT_ARLO) || ((IT) == I2C_IT_BERR) || ((IT) == I2C_IT_TXE) ||   \
   ((IT) == I2C_IT_RXNE) || ((IT) == I2C_IT_STOPF) ||                          \
   ((IT) == I2C_IT_ADD10) || ((IT) == I2C_IT_BTF) || ((IT) == I2C_IT_ADDR) ||  \
   ((IT) == I2C_IT_SB))
/**
 * @}
 */
...
/*========================================
                     ...
                     I2C Master Events (Events grouped in order of
   communication)
                                                        ==========================================*/
/**
 ...
 * that a slave acknowledges his address. If an acknowledge is sent on the bus,
 * one of the following events will be set:
 *
 ...
 *
 *  3) In case of 10-Bit addressing mode, the master (just after generating the
 * START and checking on EV5) has to send the header of 10-bit addressing mode
 * (I2C_SendData() function). Then master should wait on EV9. It means that the
 * 10-bit addressing header has been correctly sent on the bus. Then master
 * should send the second part of the 10-bit address (LSB) using the function
 * I2C_Send7bitAddress(). Then master should wait for event EV6.
 *
 */
...
#define I2C_EVENT_MASTER_MODE_ADDRESS10                                        \
  ((uint32_t)0x00030008) /* BUSY, MSL and ADD10 flags */
 ...
 *  and BTF flag at the same time (ie. (I2C_EVENT_MASTER_BYTE_RECEIVED |
 ...
 *   if the General Call is enabled for the peripheral (using function
 ...
 * similar. The second one can optionally be used when the user software doesn't
 * guarantee the EV3 is managed before the current byte end of transfer.
 ...
 * slave has to stop sending data bytes and expect a Stop condition on the bus.
 *
 *  @note In case the  user software does not guarantee that the event EV2 is
 *  managed before the current byte end of transfer, then user may check on EV2
 *  and BTF flag at the same time (ie. (I2C_EVENT_SLAVE_BYTE_RECEIVED |
 * I2C_FLAG_BTF)). In this case the communication may be slower.
 *
 */
...
#define I2C_EVENT_SLAVE_BYTE_RECEIVED                                          \
  ((uint32_t)0x00020040) /* BUSY and RXNE flags */
/* --EV4  */
...
#define I2C_EVENT_SLAVE_BYTE_TRANSMITTED                                       \
  ((uint32_t)0x00060084) /* TRA, BUSY, TXE and BTF flags */
#define I2C_EVENT_SLAVE_BYTE_TRANSMITTING                                      \
  ((uint32_t)0x00060080) /* TRA, BUSY and TXE flags */
/* --EV3_2 */
 ...
 * flags), the I2C_CheckEvent() function may return SUCCESS despite the
 * communication hold or corrupted real state. In this case, it is advised to
 * use error interrupts to monitor the error events and handle them in the
 ...
 * (I2C_IT_ERR).
 *          - I2Cx_ER_IRQHandler() which is called when the error interrupt
 * occurs. Where x is the peripheral instance (I2C1, I2C2 ...)
 *          - I2C_GetFlagStatus() or I2C_GetITStatus() to be called into
 * I2Cx_ER_IRQHandler() in order to determine which error occurred.
 *          - I2C_ClearFlag() or I2C_ClearITPendingBit() and/or
 * I2C_SoftwareResetCmd() and/or I2C_GenerateStop() in order to clear the error
 * flag and source, and return to correct communication status.
 *
 *
 *  2) Advanced state monitoring:
 *     Using the function I2C_GetLastEvent() which returns the image of both
 * status registers in a single word (uint32_t) (Status Register 2 value is
 * shifted left by 16 bits and concatenated to Status Register 1).
 *     - When to use:
 *       - This function is suitable for the same applications above but it
 * allows to overcome the limitations of I2C_GetFlagStatus() function (see
 * below). The returned value could be compared to events already defined in the
 *         library (stm32f10x_i2c.h) or to custom values defined by user.
 *       - This function is suitable when multiple flags are monitored at the
 * same time.
 *       - At the opposite of I2C_CheckEvent() function, this function allows
 * user to choose when an event is accepted (when all events flags are set and
 * no other flags are set or just when the needed flags are set like
 ...
 *     - Limitations:
 *        - When calling this function, the Status register is accessed. Some
 * flags are cleared when the status register is accessed. So checking the
 * status of one Flag, may clear other ones.
